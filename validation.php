...
if ($transaction_id) 
{
    ...
    if(Tools::getValue('id'))
    {
        ...
        $currency =  strtolower((new Currency((int)$cart->id_currency))->iso_code);

        $thisCustomer = (array) Tools::jsonDecode(Tools::getValue('customer'), true);
        $hash = Tools::getValue('response_hash');
        $total = (float) number_format($cart->getOrderTotal(true, 3), 2, '.', '');
       
        $thisHash = ((string)$merchantID.(string)Tools::getValue('payment_ref').(string)$thisCustomer["ref"].$total.$currency.(string)Tools::getValue('status').(string)$secret);
